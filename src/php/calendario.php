<?php
    ...
    $pagina = new newPage("../html/calendario.html", "Ottieni chip", "Ottieni il tuo chip", "Pagina prenotazione chip");
    ...
    if(isset($_GET['data']) && !empty($_GET['data'])){
        ...
        if(isset($valori[0]) && isset($valori[1])){
            $mese = $valori[0];
            $anno = $valori[1];
            ...
            if($mese>=1 && $mese <=12 && $anno>=date('Y')){ //Controllo sui valori della data
                ...
                $pagina->modificaHTML("{annoPrenotazione}", $anno);
                ...
                if($risultato == null)
                    ...
                    if(mysqli_num_rows($risultato) > 0){
                        ...
                        foreach( $risultato as $row){
                            ...
                            $placeHolderData .= "<li><a href=\"prenotazione.php?data=".$row['data']."\">".$giorno ."</a></li>";
                        }
    ...
    if($dateTabella == null){
        ...
        foreach($dateTabella as $row){
            if(isset($mese) && isset($anno) && $row['mese'] == $mese && $row['anno'] == $anno)
                $selezionato = "disabled selected";
            else
                $selezionato = "";

            $dateMenu .= "<option value=\"".$row['mese']."-".$row['anno']."\"".$selezionato."> ".$arrayMesi[$row['mese']]." ".$row['anno']." </option>";
        }
